...
void
Race::draw_blobs (const std::vector<Race::StepperBlob> &blobs)
{
...
void
Race::move_blobs (std::vector<Race::StepperBlob> &blobs)
{
  for (auto &blob : blobs)
    {
      blob.step ();
...
void
Race::race (std::vector<Race::StepperBlob> &blobs)
{
...
void
Race::race (std::vector<std::unique_ptr<Blob> > &blobs)
{
  using namespace std::chrono;
  const int max = 3;
  std::cout << "\x1B[2J\x1B[H";
...
void
Race::move_blobs (std::vector<std::unique_ptr<Race::Blob> > &blobs)
{
  for (auto &blob : blobs)
    {
      blob->step ();
...
void
Race::draw_blobs (const std::vector<std::unique_ptr<Race::Blob> > &blobs)
{
  const int bag_height = 3;
  for (int y = 8; y >= 0; --y)
    {
