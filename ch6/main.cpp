...
check_properties ()
{
  ...
  Race::RandomBlob   rnd_blob{ std::default_random_engine{ rd () }, std::uniform_int_distribution{ 0, 4 } };
  rnd_blob.step ();
  assert (rnd_blob.total_steps () >= 0);

  Race::RandomBlob another_rnd_blob{ std::default_random_engine{ rd () }, std::poisson_distribution{ 2 } };
  ...
  for (int i = 0; i < number / 2; ++i)
    {
      ...
      blobs.emplace_back (
          std::make_unique<RandomBlob<std::default_random_engine, std::uniform_int_distribution<int> > > (
