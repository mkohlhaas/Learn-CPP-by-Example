...
{
  std::size_t
  operator() (state_t const &state) const noexcept
  {
...
public:
  ...
  last_choices_t
  choices (const state_t &key) const
  {
  ...
  void
  update (const state_t &key, const Choice &turn_changed)
  {
    if (auto it = state_lookup.find (key); it != state_lookup.end ())
      {
...
Choice
prediction_method (const last_choices_t &choices)
{
...
template <std::invocable<> T, typename U> class MindReader
{
  State state_table;
  ...
  bool
  update_prediction (int player_choice)
  {
    bool   guessing = false;
...
template <typename Promise> struct coro_deleter
{
  void
  operator() (Promise *promise) const noexcept
  {
...
template <typename T> using promise_ptr = std::unique_ptr<T, coro_deleter<T> >;
...
struct Task
{
  ...
  struct promise_type
  {
    ...
    std::suspend_always
    yield_value (std::pair<int, int> got)
    {
